...
import { extractPromotionsFromFile } from '../services/geminiService';

import { PromoRule } from '../types';

interface PromotionsUploadModalProps {
  onClose: () => void;
  onPromosUpdated: (newPromos: PromoRule[]) => void;
}

export default function PromotionsUploadModal({ onClose, onPromosUpdated }: PromotionsUploadModalProps) {
  ...
  const handleProcess = async () => {
    ...
    try {
      const promos = await extractPromotionsFromFile(file);
      if (Object.keys(promos).length === 0) {
        throw new Error("No se detectaron promociones válidas que coincidan con la base de datos.");
      }
      onPromosUpdated(promos);
      setSuccess(true);
      setTimeout(() => {
        onClose();
      }, 2000);
    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
      ...
      >
        <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-800/20">
          <div>
            <h2 className="text-lg font-black text-white tracking-tight leading-none mb-1">Subir Promociones</h2>
        ...
        <div className="p-6 flex flex-col gap-4">
          ...
          {!file ? (
            ...
            >
              ...
              <div className="text-center">
                ...
                <span className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest">O arrastra el cuadro de promociones</span>
              </div>
            </button>
          ) : (
          ...
          {success ? (
             <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex flex-col items-center justify-center gap-2 text-emerald-400">
               ...
               <span className="text-sm font-black tracking-tight">¡Promociones actualizadas!</span>
             </div>
          ) : (
            <button
